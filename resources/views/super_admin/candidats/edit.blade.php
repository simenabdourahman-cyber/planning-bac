...
<title>Modifier Candidat — BAC 2026</title>
...
.btn-danger{background:linear-gradient(135deg,var(--rouge),var(--rouge-f));color:#fff;box-shadow:0 4px 14px rgba(192,57,43,0.3);}
...
/* INFO CANDIDAT */
.cand-info{background:linear-gradient(135deg,var(--bleu-f),#0D1F4A);border-radius:16px;padding:20px 24px;margin-bottom:24px;display:flex;align-items:center;gap:18px;box-shadow:0 8px 32px rgba(15,36,81,0.25);}
.ci-av{width:56px;height:56px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;color:#fff;font-size:20px;flex-shrink:0;border:3px solid rgba(255,255,255,0.2);}
.ci-nom{font-family:'Playfair Display',serif;font-size:18px;font-weight:700;color:#fff;}
.ci-mat{font-family:'DM Mono',monospace;font-size:11px;color:rgba(255,255,255,0.5);margin-top:3px;}
.ci-badge{margin-left:auto;background:rgba(245,200,66,0.15);border:1px solid rgba(245,200,66,0.3);color:var(--or);padding:5px 14px;border-radius:20px;font-size:11px;font-weight:700;}
...
.form-head{padding:20px 28px;border-bottom:1px solid var(--bordure);background:rgba(26,58,108,0.03);display:flex;align-items:center;gap:14px;}
.fh-ico{width:46px;height:46px;border-radius:13px;background:rgba(26,58,108,0.1);display:flex;align-items:center;justify-content:center;font-size:21px;}
.fh-titre{font-family:'Playfair Display',serif;font-size:17px;font-weight:700;color:var(--bleu);}
...
<main class="main">
    <div class="topbar">
        <div>
            <div class="page-title">Modifier le <span>Candidat</span></div>
    ...
    <div class="content">
        ...
        {{-- INFOS CANDIDAT --}}
        <div class="cand-info fi-anim">
            @php
                $couleur = $candidat->genre === 'M' ? '#1A3A6C' : '#C0392B';
                $initiales = strtoupper(substr($candidat->nom, 0, 2));
            @endphp
            <div class="ci-av" style="background:{{ $couleur }}">{{ $initiales }}</div>
            <div>
                <div class="ci-nom">{{ $candidat->nom }}</div>
                <div class="ci-mat">Matricule : {{ $candidat->matricule }} · ID : #{{ $candidat->id_candidat }}</div>
            </div>
            <div class="ci-badge">✏️ Modification</div>
        </div>

        <div class="form-card fi-anim d1">
            <div class="form-head">
                <div class="fh-ico">✏️</div>
                <div>
                    <div class="fh-titre">Modifier les informations</div>
                    <div class="fh-desc">Modifiez les données du candidat — BAC 2026</div>
                </div>
            </div>

            <form method="POST" action="{{ route('super_admin.candidats.update', $candidat->id_candidat) }}">
                @csrf
                @method('PUT')
                <div class="form-body">
                        ...
                        <div class="fg">
                            ...
                            <div class="fw">
                                ...
                                <input type="text" name="nom" class="fi @error('nom') err @enderror"
                                    value="{{ old('nom', $candidat->nom) }}" required>
                            </div>
                        ...
                        <div class="fg">
                            ...
                            <div class="fw">
                                ...
                                <input type="date" name="date_naissance" class="fi @error('date_naissance') err @enderror"
                                    value="{{ old('date_naissance', $candidat->date_naissance) }}" required>
                            </div>
                    ...
                    <div class="genre-grid">
                        @php $genre = old('genre', $candidat->genre); @endphp
                        <div class="genre-card {{ $genre === 'M' ? 'selected-m' : '' }}" onclick="selectGenre(this,'M')">
                            <input type="radio" name="genre" value="M" {{ $genre === 'M' ? 'checked' : '' }}>
                            <div class="gc-ico">👦</div>
                            <div class="gc-nom" style="color:var(--bleu)">Masculin</div>
                        </div>
                        <div class="genre-card {{ $genre === 'F' ? 'selected-f' : '' }}" onclick="selectGenre(this,'F')">
                            <input type="radio" name="genre" value="F" {{ $genre === 'F' ? 'checked' : '' }}>
                            <div class="gc-ico">👧</div>
                        ...
                        <div class="fg">
                            ...
                            <div class="fw">
                                ...
                                <input type="text" name="Telephone" class="fi @error('Telephone') err @enderror"
                                    value="{{ old('Telephone', $candidat->Telephone) }}" required>
                            </div>
                        ...
                        <div class="fg">
                            ...
                            <div class="fw">
                                ...
                                <input type="email" name="email" class="fi @error('email') err @enderror"
                                    value="{{ old('email', $candidat->email) }}" required>
                            </div>
                            ...
                            <div class="fw">
                                ...
                                <select name="ville" class="fi" required>
                                    ...
                                    @foreach(['Djibouti-Ville','Ali-Sabieh','Dikhil','Tadjourah','Obock','Arta'] as $v)
                                        <option value="{{ $v }}" {{ old('ville', $candidat->ville) == $v ? 'selected' : '' }}>{{ $v }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="fg">
                            ...
                            <div class="fw">
                                ...
                                <input type="text" name="pays" class="fi"
                                    value="{{ old('pays', $candidat->pays) }}" required>
                            </div>
                        </div>
                    </div>
                                ...
                                <select name="id_serie" class="fi">
                                    ...
                                    @foreach($series as $s)
                                        <option value="{{ $s->id_serie }}"
                                            {{ old('id_serie', $candidat->id_serie) == $s->id_serie ? 'selected' : '' }}>
                                            {{ $s->code }} — {{ $s->intitule }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="id_examen" class="fi">
                                    ...
                                    @foreach($examens as $e)
                                        <option value="{{ $e->id_examen }}"
                                            {{ old('id_examen', $candidat->id_examen) == $e->id_examen ? 'selected' : '' }}>
                                            {{ $e->code }} — {{ $e->intitule }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="fw">
                                ...
                                <select name="id_classe" class="fi">
                                    ...
                                    @foreach($classes as $cl)
                                        <option value="{{ $cl->id_classe }}"
                                            {{ old('id_classe', $candidat->id_classe) == $cl->id_classe ? 'selected' : '' }}>
                                            {{ $cl->code }} — {{ $cl->intitule }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="fw">
                                ...
                                <select name="id_specialite" class="fi">
                                    ...
                                    @foreach($specialites as $sp)
                                        <option value="{{ $sp->id_specialite }}"
                                            {{ old('id_specialite', $candidat->id_specialite) == $sp->id_specialite ? 'selected' : '' }}>
                                            {{ $sp->code }} — {{ $sp->intitule }}
                                        </option>
                                    @endforeach
                                </select>
